...
tab1_layout = [
    [
        sg.Column([
            ...
            [sg.Radio("Audio Steganography", group_id='stego_radios', key='audio_radio', default=True,
                      enable_events=True)],
            [sg.Radio("Image Steganography", group_id='stego_radios', key='image_radio', default=False,
                      enable_events=True)],
            [sg.Radio("Do not use Steganography", group_id='stego_radios', key='no_steganography_radio', default=False,
                      enable_events=True)],
            ...
            [sg.Column([[sg.Input(key='public_key', disabled_readonly_background_color='black'),
                         sg.FileBrowse(key='public_key_browse', file_types=(('PEM Files', '*.pem'),))]],
                       key='public_key_row')],
            [sg.Text(text='', key='public_key_message', size=(40, 2))],

            [sg.Text(text="Stego file", key='stego_heading')],
            [sg.Column([[sg.Input(key='stego_file', disabled_readonly_background_color='black'),
                         sg.FileBrowse(key='stego_browse',
                                       file_types=(('PNG Files', '*.png'), ('WAV Files', '*.wav')))]],
                       key='stego_row')],
        ...
        sg.Column([
            ...
            [sg.Column([[sg.Input(key='file', disabled_readonly_background_color='black', disabled=True),
                         sg.FileBrowse(key='file_browse', disabled=True)]], key='file_row')],
...
tab2_layout = [
    [
        sg.Column([
            ...
            [sg.Column([[sg.Input(key='encrypted_file'), sg.FileBrowse(key='encrypted_file_browse')]],
                       key='encrypted_file_row')],
            [sg.Text(text='', key='encrypted_file_message', size=(40, 2))],

            [sg.Text(text="Private key", key='private_key_title')],
            [sg.Column([[sg.Input(key='private_key'),
                         sg.FileBrowse(key='private_key_browse', file_types=(('PEM Files', '*.pem'),))]],
                       key='private_key_row')],
            [sg.Text(text='', key='private_key_message', size=(40, 2))],

            [sg.Text(text="Output folder", key='output_decrypt_dir_title')],
            [sg.Column([[sg.Input(key='output_decrypt_dir', disabled_readonly_background_color='black'),
                         sg.FolderBrowse(key='output_decrypt_dir_browse')]], key='output_decrypt_dir_row')],
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        ...
        if values['image_radio' or 'audio_radio']:
        ...
        try:
            ...
            if values['rsa_radio']:
                ...
                if values['image_radio'] or values['audio_radio']:
                    if os.path.exists(values['output_dir']) and os.path.exists(values['public_key']) and os.path.exists(values['stego_file']):
                        ...
                        if values['text_radio']:
                            ...
                            if values['audio_radio']:
                                asg.AudioEncode(values['stego_file'], encrypted, values['output_dir'] + '/song_embedded.wav')
                                window['encrypt_message_text'].update("Successfully Encoded", text_color='green')

                            elif values['image_radio']:
                                imgstego.Encode(values['stego_file'], encrypted, values['output_dir'] + '/image_embedded.png')
                                window['encrypt_message_text'].update("Successfully Encoded", text_color='green')
                ...
                if values['image_radio'] or values['audio_radio']:
                    if os.path.exists(values['output_dir']) and os.path.exists(values['public_key']) and os.path.exists(values['stego_file']):
                        ...
                        if values['text_radio']:
                            ...
                            if values['audio_radio']:
                                asg.AudioEncode(values['stego_file'],bytes.hex(header) + bytes.hex(sessionKeyEncrypted) + bytes.hex(nonce) + bytes.hex(tag) + bytes.hex(ciphertext), values['output_dir'] + '/song_embedded.wav')
                                window['encrypt_message_text'].update("Successfully Encoded", text_color='green')

                            else:
                                imgstego.Encode(values['stego_file'], bytes.hex(header) + bytes.hex(sessionKeyEncrypted) + bytes.hex(nonce) + bytes.hex(tag) + bytes.hex(ciphertext),values['output_dir'] + '/image_embedded.png')
                                window['encrypt_message_text'].update("Successfully Encoded", text_color='green')
            ...
            elif values['no_encryption_radio'] and values['image_radio'] or ['audio_radio']:
                ...
                if os.path.exists(values['stego_file']):
                        ...
                        if values['text_radio']:
                            ...
                            if values['audio_radio']:
                                asg.AudioEncode(values['stego_file'], 'plnt' + values['message'], values['output_dir'] + '/song_embedded.wav')
                                window['encrypt_message_text'].update("Successfully encrypted", text_color='green')

                            elif values['image_radio']:
                                imgstego.Encode(values['stego_file'], 'plnt' + values['message'], values['output_dir'] + '/image_embedded.png')
                                window['encrypt_message_text'].update("Successfully Encoded", text_color='green')
                            ...
                            if values['audio_radio']:
                                asg.AudioEncode(values['stego_file'],'plnf' + str(len(fileExt)) + fileExt + bytes.hex(file), values['output_dir'] + '/song_embedded.wav')
                                window['encrypt_message_text'].update("Successfully Encoded", text_color='green')

                            elif values['image_radio']:
                                imgstego.Encode(values['stego_file'], 'plnf' + str(len(fileExt)) + fileExt + bytes.hex(file), values['output_dir'] + '/image_embedded.png')
                                window['encrypt_message_text'].update("Successfully Encoded", text_color='green')
                        ...
                        if values['rsa_radio']:
                            ...
                            if values['text_radio']:
                                with open(values['output_dir'] + '/encrypted', 'wb') as outputFile:
                                    ...
                                    window['encrypt_message_text'].update("Successfully Done", text_color='green')
            ...
            window['encrypt_message_text'].update("Unable to Encode\n" + str(exception), text_color='red')
            ...
            if str(values['encrypted_file']).split('.')[-1].lower() != 'png':
                if not os.path.exists(values['private_key']):
                    ...
                    if str(file[0:4], 'utf-8') == 'enct':
                        ...
                        window['decrypt_message'].update("Unable to decrypt file 1\n" + "unknown file", text_color='red')
                ...
                if os.path.exists(values['encrypted_file']):
                    ...
                    file = asg.AudioDecode(values['encrypted_file'])
                    try:
                    ...
                    if file[0:4] == 'plnt':
                        ...
                        if file[0:4] == 'enct':
                            ...
                            if str(filefromhex[0:14], 'utf-8') == '12enc_text_aes':
                                ...
                                decrypted, header = crypto.DecryptAES(
                                    values['output_decrypt_dir'] + '/aes_file_fromhex_temp', values['private_key'])
                                fileExt = header[15:]
                                with open(values['output_decrypt_dir'] + '/decrypted.' + fileExt, 'wb') as outputFile:
                                    outputFile.write(bytes.fromhex(str(decrypted, 'utf-8')))
                        ...
                        window['decrypted_message'].update('')
                        window['decrypt_message'].update("Unable to decrypt file 2\n" + "unknown file", text_color='red')

        except Exception as exception:
            print(exception)
            window['decrypted_message'].update('')
            window['decrypt_message'].update("Unable to decrypt file 3\n" + str(exception), text_color='red')
